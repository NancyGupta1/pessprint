...
import {
  ...
  currentDate,
  getData,
} from "../../Service/fetchnodeadminservices";
...
export default function SubCategory(props) {
  ...
  const fetchAllSubCategory = async (cid) => {
    ...
    var result = await postData(
      "subcategory/display_all_subcategorylist",
      body
    );
  ...
  const resetValue = () => {
    ...
    setProductpicture({ bytes: "", fileName: "" });
  };
  const handleSubmit = async () => {
    ...
    if (err == false) {
      ...
      var result = await postData("product/products_submit", formData);
